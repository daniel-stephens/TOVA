...
if [ "ES" = "true" ]; then
  sudo docker compose build --no-cache api web solr-api || sudo docker compose build api web solr-api
  sudo docker compose up -d api web postgres solr-api solr zoo solr_config || sudo docker compose up -d
else
  sudo docker compose build --no-cache api web || sudo docker compose build api web
  sudo docker compose up -d api web postgres || sudo docker compose up -d
fi
EOS

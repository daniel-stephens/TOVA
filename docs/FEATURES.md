...
- **Web UI** — Django-based dashboard for corpora, training, model exploration, and inference.
...
- **CLI**: With an editable install, `python -m tova.cli.main train run …`; from a checkout without install, `python -m src.tova.cli.main train run …` (see [readme.md](../readme.md)).
- **API**: `POST /train/json` with a JSON body (model name, inline records, `text_col`, optional `corpus_id`, training params, `config_path`). Returns `job_id` and `model_id`; poll `GET /status/jobs/<job_id>`.
...
- **CLI**: Same module pattern as training (`python -m tova.cli.main` or `python -m src.tova.cli.main`).
- **API**: `POST /infer/json` with `model_id`, document records, and `text_col` (and optional `config_path`). Returns `job_id`; poll `GET /status/jobs/<job_id>` for `result.thetas`.
...
- **Topic rename**: The Django app exposes `POST /api/models/<model_id>/topics/<topic_id>/rename` and `GET /api/models/<model_id>/topics/renames`, which **proxy** to the FastAPI service at `POST /data/models/<model_id>/topics/<topic_id>/rename` and `GET /data/models/<model_id>/topics/renames`. Ensure those `/data/...` routes are implemented on the API if you rely on persisted renames (the reference `data_handling` router may not define them yet).
...
- **Models**: List, get by ID, delete. Topic renames are intended to be stored via the API paths noted in §7.4 when implemented.
...
|----------------|--------------------------------------------------------------------------|
| **Health**     | `GET /health`                                                            |
| **Status**     | `GET /status/jobs/<job_id>`, `DELETE /status/jobs/<job_id>`, `GET /status/sse_logs` (SSE) |
| **Data**       | Corpora, models, datasets, draft promotion under `/data`                  |
| **Training**   | `POST /train/json`; `GET`/`POST /train/corpus/<corpus_id>/tfidf/` (corpus exploration) |
| **Inference**  | `POST /infer/json`                                                      |
...
- **Model registry**: `static/config/modelRegistry.json` — maps model names to Python class paths under `traditional` and `llm_based`. **Version 1** product and training flows use **traditional** entries only; `llm_based` is kept for future use and is out of scope for V1 training/inference.
- **User overrides**: Stored in DB; merged with `config.yaml` for the current user when running training or labelling.
- **Environment**: e.g. `.env` for `OPENAI_API_KEY`, `DRAFTS_SAVE`, `API_BASE_URL`, `DJANGO_SECRET_KEY`, Okta, etc.

---

## 11. Deployment

- **CLI**: Install the package (e.g. `pip install -e .`) and use `python -m tova.cli.main`, or run `python -m src.tova.cli.main` from the repo root; config and registry paths are relative to the process.
- **Library**: Use `tova` as a dependency; call core dispatchers and model classes directly.
- **Docker**: Makefile-driven `docker compose` — **API** (default host port **11000**, overridable via `API_PORT`), **Web** (**8080**, `WEB_PORT`), optional **Solr API** (**8001**), **Solr** (**8983**), **Postgres** (**5432**), **Zookeeper** (client **2181**, plus an extra mapped port in compose). See [readme.md](../readme.md) for `make up`, `make down`, and service roles.

---

## 12. Out of scope for Version 1

- **LLM-based topic models**: TopicGPT, OpenTopicRAG, or any topic model whose main implementation is LLM-based. Class paths exist under `llm_based` in `modelRegistry.json`, but they are **not** part of the Version 1 training/inference flow or primary UI.

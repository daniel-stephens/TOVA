...
class CTMTMmodel(TradTMmodel):
    ...
    def train_core(
        ...
        self._logger.debug(f"train_data is set: {hasattr(self, '_train_data') and self._train_data is not None}")
        self._logger.debug(f"embeddings is set: {hasattr(self, '_embeddings') and self._embeddings is not None}")
        ...
        # hidden_sizes may arrive as a JSON string (e.g. "[100,100]") when sent
        # from the UI form, which JSON.stringifies array values before sending.
        ...
        if self.loss_weights is not None:
            ctm_kwargs["loss_weights"] = self.loss_weights
        self._model = CombinedTM(**ctm_kwargs)
        self._model.qt = self.qt
        ...
        thetas = np.asarray(self._model.get_doc_topic_distribution(tr_ds))
        self._logger.info(f"Thetas shape: {thetas.shape}")

        betas = np.asarray(self._model.get_topic_word_distribution())
    ...
    def print_topics(self, verbose: bool = False) -> list:
        topics_dict = self._model.get_topics(self.topn)  # {k: [w1, w2, ...]}
    ...
    def infer_core(self, infer_data, df_infer, embeddings_infer):
        ...
        thetas = np.asarray(self._model.get_doc_topic_distribution(ho_ds))
    ...
    def from_saved_model(cls, model_path: str):
        ...
        with model_pkl.open('rb') as f: obj._model = pickle.load(f)
        ...
        if qt_pkl.exists():
            ...
            if hasattr(obj._model, "qt") is False or obj._model.qt is None:
                obj._model.qt = obj.qt
        else:
            obj.qt = getattr(obj._model, "qt", None)
            if obj.qt is None:
                obj._logger.warning(

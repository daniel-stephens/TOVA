...
def prompt_gpt(prompt, model='gpt-4-2024-04-09',temperature=0.0, max_tokens=12000, top_p=0.3, local_model=None, tokenizer=None):
    if model == 'gpt-4-2024-04-09':
        client = AzureOpenAI(
            azure_endpoint = 'https://dil-research-sweden-central.openai.azure.com/',
            api_key='0df7237e0fc243a8830a23b2dbba2dcb',
                # api_key='a6526eab9d2042519d662014263a0465',
        ...
        client = AzureOpenAI(
            azure_endpoint = 'https://dil-research-gpt4.openai.azure.com/',
            api_version = '2023-12-01-preview',
            api_key = 'a6526eab9d2042519d662014263a0465'
        )
        ...
        client = AzureOpenAI(
            azure_endpoint="https://dil-research.openai.azure.com/",  
        ...
        return output.choices[0].message.content
    elif model == 'llama' or model == 'mistral':
        # Encode the prompt into tokens
        input_ids = tokenizer.encode(prompt, return_tensors='pt').to('cuda')

        # Generate text using the model
        outputs = local_model.generate(input_ids, max_length=5012, num_return_sequences=1)

        # Decode the generated tokens to text
        generated_text = tokenizer.decode(outputs[0], skip_special_tokens=True)

        return generated_text
    elif model == 'quantized_llama' or model == 'quantized_mistral':
        input_ids = tokenizer.encode(prompt, return_tensors="pt").to('cpu')

        with torch.no_grad():  # Ensure no gradients are computed
            outputs = local_model.generate(input_ids, max_length=5012)
...
def api_call(prompt, deployment_name, temperature, max_tokens, top_p):
    ...
    if deployment_name in ["gpt-35-turbo", "gpt-4", "gpt-3.5-turbo"]:
        ...
        '''
        Always use GPT-4o for generation
        '''
        answer = prompt_gpt(prompt, model='gpt-4o', temperature=float(temperature), max_tokens=int(max_tokens), top_p = float(top_p))
        return answer
